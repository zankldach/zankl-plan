...
def get_year_kw(year: int = None, kw: int = None) -> tuple:
    today = date.today()
    y = year or today.isocalendar()[0]
    w = kw or today.isocalendar()[1]
    return int(y), int(w)

def auto_view_target(now: datetime = None) -> tuple:
    """
    (year, kw) für Viewer:
...
def admin_peek_week_dbg(standort: str, year: int, kw: int):
...
async def set_cell(request: Request, data: dict = Body(...), standort_q: str = Query(None, alias="standort")):
    conn = get_conn(); cur = conn.cursor()
    try:
...
# ---------------- VIEW (Read-only) – gleiche Week-Logik + Freitag-12-Regel ----------------
...
# ---------------- Kleinbaustellen (nur dieser Teil NEU/erweitert) ----------------
...
async def _klein_read_payload(request: Request, data_hint: dict = None):
    """
    Akzeptiert JSON, Form, Plaintext und GET-Query.
    Versteht alias-Felder:
      - row_index | row | index | i
      - text | value | v | t
    Standort aus Body/Query/Headers; sonst via resolve_standort(Referer).
    """
    raw = {}
    ...
    if "x-row-index" in hdr and ("row_index" not in raw and "row" not in raw and "index" not in raw and "i" not in raw):
...
# Mehrfach-Alias (GET/POST), damit egal ist, welchen Endpunkt das Frontend nutzt
...
async def klein_upsert(request: Request, data: dict = Body(None)):
    try:
        st, ri, txt = await _klein_read_payload(request, data)
        _klein_upsert(st, ri, txt)
        return {"ok": True, "standort": st, "row_index": ri, "text": txt}
    except Exception:
