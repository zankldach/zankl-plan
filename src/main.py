...
def get_year_kw(year: int | None, kw: int | None):
    today = date.today()
    return (year or today.year, kw or today.isocalendar()[1])

def load_week_plan(year: int, kw: int, standort: str):
    conn = get_conn(); cur = conn.cursor()
    try:
        ...
        if not plan:
            ...
            conn.commit()
            cur.execute("SELECT * FROM week_plans WHERE id=?", (cur.lastrowid,))
            plan = cur.fetchone()
        return plan
    finally:
        conn.close()

def load_week_cells(plan_id: int):
    conn = get_conn(); cur = conn.cursor()
    out = {}
    try:
        cur.execute("SELECT row_index, day_index, text FROM week_cells WHERE week_plan_id=?", (plan_id,))
        for r in cur.fetchall():
            out[(r["row_index"], r["day_index"])] = r["text"] or ""
        return out
    finally:
        conn.close()

def load_employees_by_standort(standort: str):
    conn = get_conn(); cur = conn.cursor()
    try:
...
def view_week(
    ...
    return templates.TemplateResponse(
        "week_view.html",
        {
            ...
            "cells": cells,
            "employees": employees,
            "four_day_week": bool(plan["four_day_week"]) if plan else False,
